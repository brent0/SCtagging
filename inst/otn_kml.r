...
dir =  file.path( "E:", "OTN", "detections", "zsc_matched_detections_2019.csv" )
da = rbind(da, read.csv( dir, sep=",", header=T, colClasses = "character" ))
#                           #
...
dir = file.path( "E:", "OTN", "detections", "zsc_matched_detections_on_other_deployments_2019.csv")
da = rbind(da, read.csv( dir, sep=",", header=T, colClasses = "character" ))
...
if(length(ind)>0)
  da = da[-ind,]
ind = which(da$local_area == "Malpeque Bay")
if(length(ind)>0)
  da = da[-ind,]
ind = which(da$local_area == "Various locations")
if(length(ind)>0)
  da = da[-ind,]
ind = which(da$local_area == "Saint John River")
if(length(ind)>0)
  da = da[-ind,]
ind = which(da$local_area == "Navy Island")
if(length(ind)>0)
  da = da[-ind,]
...
ind = which(as.numeric(da$longitude) < -70)
if(length(ind)>0)
  da = da[-ind,]

ind = which(as.numeric(da$latitude) < 42)
if(length(ind)>0)
  da = da[-ind,]
